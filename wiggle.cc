...
// Function for producing a fast cosine oscillation

Double_t oscillate1(Double_t x){

  Float_t time = x;
  double A     = 0.005;               // Amplitude
  double omega = 2 * M_PI / (2000);  // short wavelength

  Double_t oscillate1 = 0.9 * ( 1 + A * cos(omega * time));
...
int main(){
  ...
  Double_t Oscfunc1[int(Total_Time / BinWidth)];
  ...
  for (int ibin = 0; ibin < (Total_Time / BinWidth); ibin++){
    ...
    Oscfunc1[ibin] = oscillate1(time);        // Fast Cosine oscillation (short wavelength)
  ...
  for (int i=0; i <= (N_Pseudo_Exp); i++){
    ...
    string tFastOscname  = "osc_short_" + conv.str();
    string tFastOsctitle = "Fast Cosine Oscillation Plot " + conv.str();

    string tWiggleAFastOscname  = "PseudoA_Fast_Osc_" + conv.str();
    string tWiggleAFastOsctitle = "PseudoA Wiggle with Fast Oscillation " + conv.str();

    string tWiggleBFastOscname  = "PseudoB_Fast_Osc_" + conv.str();
    string tWiggleBFastOsctitle = "PseudoB Wiggle with Fast Oscillation " + conv.str();

    string tWiggleCFastOscname  = "PseudoC_Fast_Osc_" + conv.str();
    string tWiggleCFastOsctitle = "PseudoC Wiggle with Fast Oscillation " + conv.str();

    string tWiggleDFastOscname  = "PseudoD_Fast_Osc_" + conv.str();
    string tWiggleDFastOsctitle = "PseudoD Wiggle with Fast Oscillation " + conv.str();

    string tWiggleTotFastOscname  = "PseudoTot_Fast_Osc_" + conv.str();
    string tWiggleTotFastOsctitle = "PseudoTot Wiggle with Fast Oscillation " + conv.str();
    ...
    // Books a histogram for fast cosine oscillation

    TH1F* tFastOsc = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tFastOsc -> SetNameTitle(tFastOscname.c_str(), tFastOsctitle.c_str());
    tFastOsc -> GetXaxis() -> SetTitle("Time (ns)");
    tFastOsc -> GetYaxis() -> SetTitle("Acceptance");

    // Books a histogram for pseudo wiggleA with fast cosine oscillation

    TH1F* tWiggleAFastOsc = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleAFastOsc -> SetNameTitle(tWiggleAFastOscname.c_str(), tWiggleAFastOsctitle.c_str());
    tWiggleAFastOsc -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleAFastOsc -> GetYaxis() -> SetTitle("Number of Positrons N");

    // Books a histogram for pseudo wiggleB with fast cosine oscillation

    TH1F* tWiggleBFastOsc = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleBFastOsc -> SetNameTitle(tWiggleBFastOscname.c_str(), tWiggleBFastOsctitle.c_str());
    tWiggleBFastOsc -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleBFastOsc -> GetYaxis() -> SetTitle("Number of Positrons N");

    // Books a histogram for pseudo wiggleC with fast cosine oscillation

    TH1F* tWiggleCFastOsc = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleCFastOsc -> SetNameTitle(tWiggleCFastOscname.c_str(), tWiggleCFastOsctitle.c_str());
    tWiggleCFastOsc -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleCFastOsc -> GetYaxis() -> SetTitle("Number of Positrons N");

    // Books a histogram for pseudo wiggleD with fast cosine oscillation

    TH1F* tWiggleDFastOsc = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleDFastOsc -> SetNameTitle(tWiggleDFastOscname.c_str(), tWiggleDFastOsctitle.c_str());
    tWiggleDFastOsc -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleDFastOsc -> GetYaxis() -> SetTitle("Number of Positrons N");

    // Books a histogram for pseudo wiggleTot with fast cosine oscillation

    TH1F* tWiggleTotFastOsc = new TH1F("", "", int(Total_Time / BinWidth), 0, Total_Time);

    tWiggleTotFastOsc -> SetNameTitle(tWiggleTotFastOscname.c_str(), tWiggleTotFastOsctitle.c_str());
    tWiggleTotFastOsc -> GetXaxis() -> SetTitle("Time (ns)");
    tWiggleTotFastOsc -> GetYaxis() -> SetTitle("Number of Positrons N");
    ...
    for (int ibin = 0; ibin < (Total_Time / BinWidth); ibin++){
      ...
      tFastOsc -> SetBinContent(ibin+1, osc1);

      tWiggleAFastOsc -> SetBinContent(ibin+1, osc1 * PseudoWiggle1);
      tWiggleBFastOsc -> SetBinContent(ibin+1, osc1 * PseudoWiggle2);
      tWiggleCFastOsc -> SetBinContent(ibin+1, osc1 * PseudoWiggle3);
      tWiggleDFastOsc -> SetBinContent(ibin+1, osc1 * PseudoWiggle4);
      tWiggleTotFastOsc -> SetBinContent(ibin+1, osc1 * PseudoWiggle);
    ...
    tFastOsc -> Write();
    tWiggleAFastOsc -> Write();
    tWiggleBFastOsc -> Write();
    tWiggleCFastOsc -> Write();
    tWiggleDFastOsc -> Write();
    tWiggleTotFastOsc -> Write();
    ...
    delete tFastOsc;
    delete tWiggleAFastOsc;
    delete tWiggleBFastOsc;
    delete tWiggleCFastOsc;
    delete tWiggleDFastOsc;
    delete tWiggleTotFastOsc;
